...
object AverageTweetLength {
  ...
  def main(args: Array[String]) {
    ...
    lengths.foreachRDD((rdd, time) => {
      ...
      var count = rdd.count()
      if (count > 0) {
        //getAndAdd() in a thread safe manner
        //running counter of totalTweets
        totalTweets.getAndAdd(count)
        //reduces & gives the total number of characters present in that individual RDD - this returns just an
        //integer 
        // this keeps adding and will get represented in the thread-safe "totalChars" running counter that we have defined
        totalChars.getAndAdd(rdd.reduce((x,y) => x + y))
        
        println("Total tweets: " + totalTweets.get() + 
            //running totals of chars
            " Total characters: " + totalChars.get() + 
            //running average of chars and tweets encountered so far
